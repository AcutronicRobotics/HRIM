...
class ModulePrinter:
    ...
    def printValue(self, tabs, title, value, type=None):
        ...
        if value is None or len(value) == 0:
            ...
            if isinstance(value, list) or isinstance(value, dict):
                msg += "["
                for val in value:
                    if type is not None and type == "string":
    ...
    def printProperty(self, property, baseTabs):
        ...
        if property.type in self.dataTypes:
            type = self.dataTypes[property.type]+(
                ("[{}]".format(property.length if property.length is not None else ""))
                if property.array else ""
            )
        else:
            # if the type doesn't appear on the dataMapping file we assume it's a generated type,
            # we capitalize it to differentiate them
            type = property.type[0:1].upper()+property.type[1:]+(
                ("[{}]".format(property.length if property.length is not None else ""))
                if property.array else ""
            )
        ...
        if len(property.properties) > 0:
            ...
            if(property.fileName is not None
               and (property.fileName not in self.shownFiles or self.extend)
              ):
            ...
            self.printValue(
                baseTabs+2, "value",
                list(property.enumeration.keys())[
                    list(property.enumeration.values()).index(int(property.value))
                ]
            )
        else:
